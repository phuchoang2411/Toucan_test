...
- **Dời lịch** (sửa ngày và/hoặc sales rep trên kế hoạch mà form đang gắn vào) sẽ **di chuyển** buổi ghé thăm đó sang ngày mới và/hoặc chuyển sang rep mới thay vì tách thành một kế hoạch thứ hai — cùng id, cùng evidence, MISA được xếp lại `Queued`. Nếu đã có một buổi ghé thăm `planned` khác đúng vào (rep, ngày) đích, việc dời lịch bị từ chối (`DATE_ALREADY_PLANNED`) thay vì âm thầm gộp hai dòng và làm mất evidence của một trong hai.
...
Buổi ghé thăm lưu `currentStageSnapshot` — giai đoạn của outlet **tại thời điểm buổi ghé thăm được lên lịch**. Giai đoạn thực tế (live) của outlet có thể thay đổi trước khi buổi ghé thăm diễn ra; snapshot giữ lại hình ảnh kế hoạch tại thời điểm lập kế hoạch. Việc dời lịch hoặc chuyển rep (A1) chỉ cập nhật ngày/rep/target/mục tiêu trên dòng hiện có nhưng cố tình không cập nhật lại `currentStageSnapshot` — nó ghi lại thời điểm lập kế hoạch ban đầu, không phải lần sửa gần nhất.
...
### A9. Phân quyền: rep vs. manager, thực thi ở tầng service
Giới hạn chỉ ở UI (ẩn nút, khóa field) chỉ hướng dẫn người dùng thiện chí — bất kỳ ai mở devtools, sửa URL, hoặc gọi thẳng hàm service đều có thể bỏ qua chúng. Vì vậy prototype bổ sung một mô hình phân quyền nhẹ, với phần *thực thi* nằm ở tầng service (`src/services/*`) — chính là tầng đã trung gian hóa mọi thay đổi dữ liệu; UI phản ánh cùng quy tắc đó nhưng không phải là nguồn sự thật cho chúng.

**Vai trò (Roles).** Hai vai trò, không hơn (`src/domain/types.ts`, `USERS`):
- **Rep** — sở hữu các outlet/visit có `salesRep === chính họ`. Chỉ xem và quản lý được bản ghi của mình.
- **Manager** — xem và quản lý mọi bản ghi, và là vai trò duy nhất được phép chuyển một outlet (hoặc visit gắn với nó) sang rep khác.

**Phiên (Session).** Không có xác thực thật — `src/store/session.ts` giữ một "current user" giả lập (một mục trong `USERS`), có thể đổi qua bộ chọn "Signed in as" trên thanh nav (`UserSwitcher`). Đây là đại diện cho ngữ cảnh request đã xác thực; ở backend thật nó sẽ đến từ một session/JWT đã kiểm chứng, không phải một nút bấm phía client.

**Chính sách (Policy).** `src/domain/authz.ts` là một module thuần nhỏ — `canAccess(user, record)` (manager, hoặc `record.salesRep === user.name`) và `canReassign(user)` (chỉ manager) — được dùng theo hai cách:
- **Service khẳng định và ném lỗi** (`FORBIDDEN` / `FORBIDDEN_REASSIGN`) trước khi thay đổi dữ liệu: `outletService.save`, và mọi thao tác ghi trong `visitService` (`upsertPlanned`, `addEvidence`, `cancelVisit`, `reschedule`, `complete`). Đây là thực thi thật — áp dụng bất kể lời gọi đến từ đâu, UI hay không. Khi tạo mới, outlet của một rep sẽ tự động gán về chính họ; một rep đổi `salesRep` khi sửa (hoặc lên lịch dưới tên rep khác) sẽ bị từ chối.
- **UI gọi cùng hàm predicate đó** để khóa field sales rep, ẩn bộ lọc chỉ dành cho manager, và ẩn/hiện các nút hành động — đây là UX, không phải bảo mật; vai trò duy nhất của nó là tránh cho người dùng thiện chí chạm vào bức tường mà họ không cần chạm.

**Giới hạn phạm vi xem (View scoping).** Các trang danh sách/dashboard đọc dữ liệu qua `useScopedDB` (`src/hooks/useScopedDB.ts`), lọc `outlets`/`visits` chỉ còn bản ghi của user hiện tại trừ khi họ là manager — cùng quy tắc `canAccess`, áp dụng cho việc đọc. Các trang truy cập trực tiếp theo id (sửa outlet, chi tiết visit) coi một bản ghi tồn tại-nhưng-không-được-phép giống hệt một bản ghi không tồn tại, để một rep dò URL của rep khác không biết được liệu bản ghi đó có tồn tại hay không.

**Ghi nhận người thực hiện (Attribution).** `StageHistory.changedBy` ghi nhận người dùng *đang thao tác*, không phải rep của visit — vì vậy một manager hoàn tất visit thay cho rep sẽ được ghi nhận đúng là manager, giữ đúng ý nghĩa của audit trail.

**Thành thật về giới hạn của prototype.** Đây vẫn là một app chỉ chạy phía client, không có backend thật: `localStorage` có thể bị sửa dễ dàng, và "tầng service" chạy trong cùng tiến trình trình duyệt với UI, nên một người dùng đủ quyết tâm vẫn có thể can thiệp trực tiếp vào state. Giá trị của thiết kế này nằm ở cấu trúc — việc thực thi nằm ở ranh giới dữ liệu (services), không rải rác trong các component — và cấu trúc đó giữ nguyên khi chuyển sang backend thật: thay `session` giả lập bằng một request context đã xác thực (JWT/cookie), chuyển `authz.ts` và các kiểm tra trong service sang phía server (hoặc row-level security), còn tầng UI không cần thay đổi.

**Ngoài phạm vi của prototype này:** xác thực thật, phân quyền chi tiết hơn rep/manager, ghi log truy cập đọc, và quản lý user ở cấp admin (`USERS` là danh sách seed cứng, tương tự `SALES_REPS` trước đây).
...
- Nếu chọn "lên lịch ghé thăm": visitDate, targetStage, mục tiêu (objective) → bắt buộc; ngày trong quá khứ → cảnh báo (A5); targetStage nên khác giai đoạn hiện tại → cảnh báo, không phải lỗi.
